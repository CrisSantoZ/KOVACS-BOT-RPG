...
// RPG: Arcádia - A Era dos Reinos (Completo, Corrigido e Otimizado)
...
async function handleDistribuirPontos(chatId, sender, args) {
    ...
    // Não precisa de fichaTemporaria se validarmos o total de pontos antes de aplicar
    // ficha.atributos.pontosParaDistribuir já tem o valor correto.

    for (let i = 0; i < args.length; i += 2) {
        const atributoNome = args[i].toLowerCase().trim();
        const pontosADistribuirStr = args[i+1];
        
        if (!pontosADistribuirStr) {
